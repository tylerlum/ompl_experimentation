...
def plan(run_time, planner_type, objective_type, wind_direction, dimensions, obstacles):
    ...
    cbounds.setLow(-math.pi / 2)
    cbounds.setHigh(math.pi / 2)
    cspace.setBounds(cbounds)

    # define a simple setup class
    ss = oc.SimpleSetup(cspace)
    # ss = oc.SimpleSetup(space)
    ...
    print("getStateValidityCheckingResolution () {}".format(si.getStateValidityCheckingResolution()))
    ...
    if solved:
        ...
        controls = solution_path.getControls()
        print("State 0: {}".format(states[0]))
        print("Control 0: {}".format(controls[0]))
        for state in states:
        ...
        print(ss.haveSolutionPath())
        print(ss.haveExactSolutionPath())
        plot_path(ss.getSolutionPath(), dimensions, obstacles)
        print("***")
        print(ss.getSolutionPath().length())
        print(ss.getSolutionPath().check())
        print(ss.getSolutionPath().clearance())
